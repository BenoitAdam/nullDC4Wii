...
static void SetTextureParams(PolyParam *mod, bool decal_alpha_fix)
{
  ...
  if (!cache_valid)
  {
    ...
    // never be mistaken for a plane pair.
    pbuff->is_yuv_tev = yuv_tev_this_tex;
    if (yuv_tev_this_tex)
    {
      u8 *uv_plane = (u8*)dst + (u32)w*h;
      DCFlushRange(uv_plane, (u32)w*h); // (w/2)*h IA8 texels * 2 bytes
...
// ── FMV TEV YUV program (FMV_FORMAT_TEV) ────────────────────────────────────
...
static void tev_yuv_setup_full()
{
  ...
  // Second texcoord for the chroma plane, generated from the SAME vertex
  // TEX0 attribute — the vertex format/descriptor stays untouched.
...
// relies on: stage0 order/op, texgen count, and (when offset_fix is active)
// the offset-color add on stage 1, which the frame setup only configures once.
...
*/
  ...
  */
  ...
  const bool fmv_tev = FMV_FORMAT_TEV(); // FMV TEV YUV path, read once per frame
