...
#include <string.h>
#include <gccore.h>
#include <tuxedo/ppc/exception.h>  // PPCExcptSetHandler, PPC_EXCPT_DSI/ISI
...
static void wmmu_hw_init()
{
    ...
    PPCExcptSetHandler(PPC_EXCPT_DSI, wmmu_excpt_stub);
    PPCExcptSetHandler(PPC_EXCPT_ISI, wmmu_excpt_stub);
...
static inline bool wmmu_pc_valid(u32 pc);   // forward decl (defined below)
...
static void wmmu_dump(frame_context* ctx, const char* why)
{
    ...
    printf("\n[WMMU] FATAL %s: %s (dispatched so far: %u)\n",
           ctx->EXCPT_Number == PPC_EXCPT_DSI ? "DSI" : "ISI", why,
           wmmu_stats.dsi_dispatched);
    printf("[WMMU] PC=%08x DAR=%08x DSISR=%08x MSR=%08x\n",
           ctx->pc, ctx->dar, dsisr, ctx->MSR);
    if (wmmu_pc_valid(ctx->pc - 12) && wmmu_pc_valid(ctx->pc + 12))
    {
        // Surrounding words too: confirms whether pc really sits inside the
        // JIT's guarded window-access shape (rlwinm/cmpwi/beq — see
        // wmmu_backpatch's comment) or looks like unrelated code, which
        // would point at a stale/wrong captured pc instead of a decode bug.
        for (s32 off = -12; off <= 12; off += 4)
            printf("[WMMU] [pc%+03d]=%08x\n", off, *(u32*)(ctx->pc + off));

        const u32 op = *(u32*)ctx->pc;
        printf("[WMMU] OP=%08x ra=%u rd/rs=%u rb=%u disp=%d\n",
               op, (op >> 16) & 31, (op >> 21) & 31, (op >> 11) & 31,
               (s16)(op & 0xFFFF));
        printf("[WMMU] GPR[ra]=%08x GPR[rb]=%08x\n",
               ctx->GPR[(op >> 16) & 31], ctx->GPR[(op >> 11) & 31]);
    }
...
static void wmmu_handle_dsi(frame_context* ctx)
{
    ...
    const bool from_jit = wmmu_pc_in_cache(ctx->pc);
    ...
    if (from_jit)
        wmmu_backpatch(ctx->pc, m.size, m.dcbz);
    ...
    ctx->pc += 4;   // resume past the emulated instruction
    wmmu_depth = wmmu_depth - 1;
}
...
{
    if (ctx->EXCPT_Number == PPC_EXCPT_DSI)
        wmmu_handle_dsi(ctx);
    else
    {
        wmmu_stats.isi_seen++;
