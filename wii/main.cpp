...
// (MOV pc,lr / LDMFD ..pc); built 2026-09-14, awaiting Wii A/B against 2.
...
static void printOptionsFooter(void)
{
  ...
  printf("\033[%d;1H1-Y: Previous | 2+X: Next | alpha 0.74", rows);
}
  ...
  while (true)
  {
    ...
    else if (pressed & WPAD_BUTTON_LEFT)
    {
      switch (selectedRow) {
        ...
        case OPT_ARM7_JIT:       g_arm7_jit_preset        = (g_arm7_jit_preset        + 3) % 4; break;
    ...
    else if (pressed & WPAD_BUTTON_RIGHT)
    {
      switch (selectedRow) {
        ...
        case OPT_ARM7_JIT:       g_arm7_jit_preset        = (g_arm7_jit_preset        + 1) % 4; break;
